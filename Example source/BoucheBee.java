...
public class BoucheBee extends Gameplay {
    ...
    /** Event to call doSetup, which fills the initial board randomly. */
        ...
        /** Event to when a cell as been selected, to figure connected cells */
        private static final int EVENT_SELECT = 1;
        /** Event to remove previously selected items */
        ...
        private BevelRect mBgSprite;
        private BevelRect mSelSprite;
        ...
        private int mSelectedCellsCount;
        private Cell mCurrentSelCell;
        private long mFillChanged;

        public BoucheBee(AsqareContext context) {
                super(context);
        ...
        public String getName() {
                return getContext().getActivity().getString(R.string.title_bouchebee);
        }

        @Override
        public void create(String state) {
                // create sprites only once, then reuse the same objects
                if (!mCreated) {
                        mCreated = true;
                        ...
                        for (Color color : Color.colors1) {
                                if (color != Color.BLUE1 && color != Color.RED2) {
                                        mSprites[n++] = BevelSprite.instantiate(BevelCircle.class, color, 8, 5);
                                }
                        }
                }
        ...
        public void start() {
                ...
                if (mBoard == null) {
                        ...
                        mMoves = 0;
                        mTempStack = null;
                        mCurrentSelCell = null;
                        mSelectedCells = null;
                        mSelectedCellsCount = 0;
        ...
        private void createBoard() {
                ...
                if (mTempStack == null || mTempStack.length != n) mTempStack = new int[n];
                if (mSelectedCells == null || mSelectedCells.length != n) mSelectedCells = new Board.Cell[n];
                setBoard(mBoard);
        }
    ...
    @Override
        protected void onActivate() {
                ...
                if (sx >= 0 && sy >= 0 && sx < bx && sy < by) {
                        ...
                        if (mCurrentSelCell == null) {
                                // if there's no first selection, set it
                                mCurrentSelCell = cell;
                                mAnimThread.startBlinking(cell, AnimThread.FPS / 2);
                                mAnimThread.queueGameplayEvent(this, EVENT_SELECT);
                                getContext().vibrateTouch();
                        } else {
                                boolean is_selected = false;
                                for (int i = 0; i < mSelectedCellsCount; i++) {
                                        if (mSelectedCells[i] == cell) {
                                                is_selected = true;
                                                break;
                                        }
                                }

                                if (is_selected && mSelectedCellsCount > 1) {
                                        // trigger removal
                                        mAnimThread.queueGameplayEvent(this, EVENT_REMOVE);
                                } else {
                                        // dismiss touch & selection
        ...
        private void doSelect() {
                mAnimThread.stopAnim(mCurrentSelCell);

                mFillChanged = floodFillMark(mBoard.getBoardArray(), mCurrentSelCell);
                mAnimThread.startBlinking(mSelectedCells, mSelectedCellsCount, AnimThread.FPS / 2);
        }
        ...
        private void doRemove() {
                ...
                if (mSelectedCellsCount > 1) {
                        mAnimThread.startShrink(mSelectedCells, mSelectedCellsCount, 2, 10 /* 1s @ 10fps */);
                        num_changed = mSelectedCellsCount;
                }
                ...
                if (num_changed != 0) {
                        ...
                        updateMessage();
                        getContext().vibrateSequence(mSelectedCellsCount);
        ...
        private void doDone() {
                for (int i = 0; i < mSelectedCellsCount; i++) {
                        mAnimThread.stopAnim(mSelectedCells[i]);
                }
         ...
         * The actual finding of adjacent cells is done in subFloodFill(), which uses the
         ...
         */
        private long floodFillMark(Cell[] cells, Cell originCell) {

                int x = originCell.getX();
                int y = originCell.getY();
                Sprite s = originCell.getSprite();
                int k = y * mBx + x;
                long changed = 0;
                ...
                // we will "mark" by inserting cells in the selected cells list
                mSelectedCellsCount = 0;

                while(true) {
                        // process x/y/k
                        changed = subFloodFill(cells, changed, x, y, k, s, stack);
                        push = stack[0];
        ...
        /**
         * "inner" part of the flood-fill algorithm. See floodFillMark().
         */
        private long subFloodFill(Cell[] cells, long changed,
        ...
        /**
         * Marks one cell for removal in the bit field and add it to the selected list.
         */
        private long mark(Cell[] cells, int k1, long changed) {
                mSelectedCells[mSelectedCellsCount++] = cells[k1];
                changed |= (1L << k1);
